...
# jevkit Installer
# Usage: curl -fsSL https://raw.githubusercontent.com/demo/jevkit/main/install.sh | sh
#
# Environment variables:
#   JEVKIT_INSTALL_DIR - Installation directory (default: ~/.local/bin)
#   JEVKIT_VERSION     - Specific version to install (default: latest)

set -eu

# Configuration
GITHUB_REPO="demo/jevkit"
BINARY_NAME="jevkit"
...
detect_os() {
    case "$(uname -s)" in
        ...
        MINGW*|MSYS*|CYGWIN*)
            ...
            printf 'jevkit requires WSL (Windows Subsystem for Linux).\n\n' >&2
...
main() {
    ...
    local install_dir="${JEVKIT_INSTALL_DIR:-$DEFAULT_INSTALL_DIR}"
